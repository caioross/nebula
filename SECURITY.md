...
3. **Credential handling.** Keys for connected providers are entered as plain conversation. How they are recognized, masked in history and kept out of model context is specified in [docs/models.md](docs/models.md); what protects them once written, including on a machine with no keychain running, is in [docs/architecture.md](docs/architecture.md#vault). Both are standing audit targets.
