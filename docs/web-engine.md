...
**Layer 2 — Capability gating.** A model call that has foreign content in context runs with reduced capabilities by default: it can produce text and propose actions, but proposed actions that leave the machine (sending, submitting, purchasing, connecting) require confirmation through the canvas in Nebula's own voice — and that confirmation sentence is generated from the *structured action*, never from model prose, so a compromised generation can't write its own permission slip.
