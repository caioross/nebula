...
3. **Credential handling.** Keys for connected providers are entered as plain conversation. Recognizing one and keeping it out of history, model context and logs, wherever in a sentence it turns up, is the router's job ([docs/routing.md](docs/routing.md)); the connection flow that follows when the user is actually handing one over is in [docs/models.md](docs/models.md); what protects them once written, including on a machine with no keychain running, is in [docs/architecture.md](docs/architecture.md#vault). All three are standing audit targets.
